...
from PyPoE.poe.file.specification.errors import (
    SpecificationError,
...
class DatReader(ReprMixin):
        ...
        if casts[0][0] in (self.CastTypes.VALUE, self.CastTypes.POINTER_SELF):
            ivalue = (
                ...
                if data
                    ...
                    '<' + casts[0][2],
                    self._file_raw[offset : offset + casts[0][1]],
                )[0]
            )
            ...
            self.CastTypes.POINTER_LIST,
            self.CastTypes.POINTER,
        ):
            data = (
                data
                if data
    ...
    def read(self, raw):
        ...
        if isinstance(raw, bytes):
            ...
            raise TypeError(
                'Raw must be bytes or BytesIO instance, got %s' % type
            )
...
class RelationalReader(AbstractFileCache):
    ...
    def get_file(self, file_name):
        ...
        vf = (
            self._dv_set_value
            if df.reader.use_dat_value
            else self._simple_set_value
        )
                ...
                for i, row in enumerate(df.reader.table_data):
                    try:
                        df.reader.table_data[i][index] = vf(
                            row[index], df_other_reader, key_id, key_offset,
                        )
