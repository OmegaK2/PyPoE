...
class ContextToolbar(QToolBar):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.action_search.setStatusTip(
            self.tr('Search currently selected folder')
        )
    ...
    def _toolbar_extract(self):
        ...
        if self.parent().s_general.uncompress_dds:
            if isinstance(node.record, ggpk.DirectoryRecord):
                ...
                node.record, ggpk.FileRecord
            ) and node.name.endswith('.dds'):
    ...
    def _toolbar_search(self):
        ...
        result = node.search(
            self.regex_search.regex_compiled, search_directories=sdir
        )
        if result:
            out = []
            stop = (
                None if self.regex_search.option_full_path.isChecked() else node
            )
            for item in result:
