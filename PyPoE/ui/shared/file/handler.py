...
class DatStyle(QStyledItemDelegate):
    ...
    def _show_value(self, outstr, dat_value, format='{0}'):
        if (
            self.data_style
            or self.parent().option_dereference_pointer.isChecked()
        ):
