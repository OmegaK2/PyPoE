...
class PassiveSkillCommandHandler(ExporterHandler):
    def __init__(self, sub_parser):
        self.parser = sub_parser.add_parser(
            'passive', help='Passive skill exporter',
        )
        self.parser.set_defaults(func=lambda args: self.parser.print_help())

        self.add_default_subparser_filters(
            sub_parser=self.parser.add_subparsers(), cls=PassiveSkillParser,
        )
...
class PassiveSkillParser(parser.BaseParser):
    ...
    _COPY_KEYS = OrderedDict(
        (
            ('Id', {'template': 'id',}),
            ('PassiveSkillGraphId', {'template': 'int_id',}),
            ('Name', {'template': 'name',}),
            ('FlavourText', {'template': 'flavour_text', 'default': '',}),
            (
                'Reminder_ClientStringsKeys',
                {
                    'template': 'reminder_text',
                    'format': lambda value: '<br>'.join(
                        [x['Text'] for x in value]
                    ),
                    'default': '',
                    'condition': lambda passive: passive[
                        'Reminder_ClientStringsKeys'
                    ],
                },
            ),
            ...
            ('SkillPointsGranted', {'template': 'skill_points', 'default': 0,}),
            ...
            ('IsKeystone', {'template': 'is_keystone', 'default': False,}),
            ('IsNotable', {'template': 'is_notable', 'default': False,}),
            (
                'IsMultipleChoiceOption',
                {'template': 'is_multiple_choice_option', 'default': False,},
            ),
            (
                'IsMultipleChoice',
                {'template': 'is_multiple_choice', 'default': False,},
            ),
            ('IsJustIcon', {'template': 'is_icon_only', 'default': False,}),
            (
                'IsJewelSocket',
                {'template': 'is_jewel_socket', 'default': False,},
            ),
            (
                'IsAscendancyStartingNode',
                {'template': 'is_ascendancy_starting_node', 'default': False,},
            ),
        )
    )
        ...
        for passive in passives:
            ...
            if node and node.connections:
                data['connections'] = ','.join(
                    [
                        self.rr['PassiveSkills.dat'].index[
                            'PassiveSkillGraphId'
                        ][psg_id]['Id']
            ...
            if parsed_args.store_images and self.ggpk:
                ...
                if not (os.path.exists(dds) or os.path.exists(png)):
                    self._write_dds(
                        data=self.ggpk[passive['Icon_DDSFile']]
                        .record.extract()
                        .read(),
                        out_path=dds,
            ...
            r.add_result(
                ...
                wiki_page=[
                    {
                        'page': 'Passive Skill:'
                        + self._format_wiki_title(data['id']),
                        'condition': cond,
                    },
                ],
